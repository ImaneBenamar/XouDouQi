...
public class Elephant extends Animal {
	...
	private static final int ELEPHANT_POWER = 8;

	/**
	 * The PieceFormat is for our Board, so basically our board is String Matrix, so
	 * our piece have to be represented by a string format in order to make the
	 * board easy to be read.
	 */
	

	public String PieceFormat = "E" + player.getColor();
	
	public Elephant(Board b, Point position, Player player) {
	...
	public boolean isPossibleMove(Point p) {
		...
		if (!board.isEmpty(p) && (board.getAnimalAt(p).player.getColor() == this.player.getColor()
				|| board.getAnimalAt(p).getPower() > this.getPower()) && board.getAnimalAt(p).getPower() == 1) {
			return false;
		}
	...
	public String toString() {
		return "Elephant: Power" + this.getPower() + " " + this.player.toString();
	}
